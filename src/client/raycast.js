...
// Lights
const hemiLight = new THREE.HemisphereLight(0xffffff, 0x884844 , 0.2);
...
{   
    if(mousedown)
    {
        ...
        if(intersects[0].object.name != "Skybox" && intersects[0].object.name != "Ground")
        {   
            if(!toggle)
            {
                intersects[0].object.scale.x += 0.05
            ...
            console.log("Distance = " + camera.position.distanceTo(intersects[0].object.position))
        }
        console.log(intersects[0].object)
    }
}

 window.addEventListener('pointerdown' , RaycastOnClick);